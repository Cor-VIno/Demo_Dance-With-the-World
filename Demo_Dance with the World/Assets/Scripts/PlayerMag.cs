...
public class PlayerMag : MonoBehaviour {
    ...
    void Start() {
        ...
        soundEffectController.PlayResetMag();
        UpdateHasMagTypes();
        ...
        if (Input.GetMouseButtonDown(0) && playerHasMagTypes.Count > 0) {
            if (lookAtObjMag.magMode != E_MagMode.None) {
                if (!contactedObjs.Contains(lookAtObj) || lookAtObjMag.magMode == playerHasMagTypes.Peek()) {
                    if (lookAtObjMag.magMode == playerHasMagTypes.Peek()) {
                        soundEffectController.PlayRepulse();
                    } else {
                        soundEffectController.PlayAttract();
                    }
                ...
                soundEffectController.PlayGiveMag();
                UpdateHasMagTypes();
            }
        } else if (Input.GetMouseButtonDown(1) && playerHasMagTypes.Count < maxTypeCount) {
            if (lookAtObjMag.magMode != E_MagMode.None) {
                var temp = lookAtObjMag.TakeMagMode();
                playerHasMagTypes.Push(temp);
                if (temp == E_MagMode.N) {
                    soundEffectController.PlayGetN();
                } else {
                    soundEffectController.PlayGetS();
                }
